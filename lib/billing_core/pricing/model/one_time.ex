defmodule BillingCore.Pricing.Model.OneTime do
  @moduledoc """
  One-time charge (SPEC §9.6): a fixed `unit_price × quantity` (quantity is
  usually 1). One-time charges never prorate; without a service period they
  are point-in-time lines with no accrual fields (SPEC §9.4).

  `unit_price` is a non-negative `Decimal` in major currency units.
  """
