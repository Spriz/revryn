// Package commands implements the billingctl Cobra command tree with stable
...
// Version is the billingctl release version, stamped at build time via
// -ldflags "-X github.com/revryn/billing-core/internal/commands.Version=...".
...
// Main runs billingctl with process defaults and returns the exit code.
...
// NewRootCommand builds the billingctl command tree bound to app.
func (a *App) NewRootCommand() *cobra.Command {
	...
	root := &cobra.Command{
		...
		Long:          "billingctl administers and automates Billing Core through its public GraphQL contract (SPEC §12.2.3, INV-043).",
		Version:       Version,
		SilenceUsage:  true,
		SilenceErrors: true,
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			a.JSON = flagJSON
			a.Client = client.New(client.Config{BaseURL: a.URL, Token: a.Token, UserAgent: "billingctl/" + Version})
			return nil
		},
	}
	root.SetVersionTemplate("billingctl {{.Version}}\n")
	...
	pf.BoolVar(&flagJSON, "json", false, "emit stable machine-readable JSON (schema billingctl.v1)")
