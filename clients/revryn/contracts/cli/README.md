...
`revryn` is the official Go CLI for Billing Core (SPEC §12.2.3,
...
```sh
CGO_ENABLED=0 go build -o revryn ./cmd/revryn
```
...
CGO_ENABLED=0 go build \
  -ldflags "-X github.com/spriz/revryn/clients/revryn/internal/commands.Version=1.2.3" \
  -o revryn ./cmd/revryn
```

`make build` wraps the above; `test`, `vet`, and
...
| `--json` | — | off | Emit the stable `revryn.v1` envelope instead of human tables. |
...
```json
{"data": {...}, "correlationId": "uuid", "schema": "revryn.v1"}
```
