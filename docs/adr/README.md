...
| 026 | Go is the implementation language for `revryn` and MCP | [026-go-revryn-cli-mcp.md](026-go-revryn-cli-mcp.md) | CLI and MCP implemented in Go; Phoenix remains the domain runtime. |
...
| 031 | Credit close corrections are compensating closes | [031-credit-close-corrections-are-compensating-closes.md](031-credit-close-corrections-are-compensating-closes.md) | Never mutate a booked close; corrections are new compensating closes. |
| 032 | Repo-committed secrets, age-encrypted via fnox | [032-fnox-age-encrypted-repo-secrets.md](032-fnox-age-encrypted-repo-secrets.md) | `fnox.toml` holds the reviewable secret catalog with age-encrypted values; identities stay outside the repo. |
