#!/usr/bin/env bash
# Integrated showcase certification: Driftbord × Billing Core
...
DJANGO="$ROOT/examples/work-management-django"
DJANGO_PORT="${DJANGO_PORT:-8322}"
...
say "boot Driftbord in integrated mode"
pkill -f "manage[.]py runserver $DJANGO_PORT" 2>/dev/null || true
sleep 1
DB="$(mktemp -u /tmp/driftbord-integrated-XXXX.sqlite3)"
export DRIFTBORD_DB="$DB" DRIFTBORD_BILLING=integrated \
  BILLING_CORE_URL="$BASE_URL" BILLING_CORE_TOKEN="$TOKEN" BILLING_CORE_TEAM_ID="$TEAM"
(cd "$DJANGO" && .venv/bin/python manage.py migrate --run-syncdb >/dev/null \
  && .venv/bin/python manage.py runserver "$DJANGO_PORT" --noreload >/tmp/driftbord-integrated.log 2>&1) &
DJANGO_PID=$!
trap 'pkill -f "manage[.]py runserver $DJANGO_PORT" 2>/dev/null || true; kill $DJANGO_PID 2>/dev/null || true' EXIT
for _ in $(seq 1 30); do
  curl -sf "http://localhost:$DJANGO_PORT/login/" >/dev/null 2>&1 && break
  sleep 1
done

say "phase 1: browser certification (usage + live preview traceability)"
SPEC_OUT="$(cd "$ROOT/e2e/showcases/driftbord" && DRIFTBORD_URL="http://localhost:$DJANGO_PORT" npx playwright test --reporter=line 2>&1)" || {
...
CUSTOMER_ID="$(jq -r --arg x "driftbord-$ORG_SLUG" '.data.customers.edges[] | select(.node.externalId == $x) | .node.id' <<<"$CUSTOMERS")"

PRODUCTS="$(gql 'query($t: ID!, $f: Int){ products(teamId: $t, first: $f){ edges { node { id code } } } }' \
  "$(jq -nc --arg t "$TEAM" '{t: $t, f: 100}')")"
PRODUCT_ID="$(jq -r '.data.products.edges[] | select(.node.code == "driftbord-seat") | .node.id' <<<"$PRODUCTS")"

SUBS="$(gql 'query($t: ID!, $f: Int){ subscriptions(teamId: $t, first: $f){ edges { node { id externalId } } } }' \
  "$(jq -nc --arg t "$TEAM" '{t: $t, f: 100}')")"
SUB_ID="$(jq -r --arg x "driftbord-$ORG_SLUG" '.data.subscriptions.edges[] | select(.node.externalId == $x) | .node.id' <<<"$SUBS")"
...
MAP_C="$(gql 'mutation($i: MapCustomerToErpInput!){ mapCustomerToErp(input: $i){ __typename ... on ValidationProblem { code message } } }' \
  "$(jq -nc --arg t "$TEAM" --arg c "$CUSTOMER_ID" --arg e "$CONNECTION" \
     '{i: {teamId: $t, customerId: $c, erpConnectionId: $e, externalCustomerNumber: "9001", clientMutationId: "showcase-map-c"}}')")"
...
MAP_P="$(gql 'mutation($i: MapProductToErpInput!){ mapProductToErp(input: $i){ __typename ... on ValidationProblem { code message } } }' \
  "$(jq -nc --arg t "$TEAM" --arg p "$PRODUCT_ID" --arg e "$CONNECTION" \
     '{i: {teamId: $t, productId: $p, erpConnectionId: $e, externalProductNumber: "DRIFT-SEAT", clientMutationId: "showcase-map-p"}}')")"
